...
object UIJDLattice {
  ...
  implicit def OptionAsUIJDLattice[A: UIJDLattice]: UIJDLattice[Option[A]] = new UIJDLattice[Option[A]] {
    ...
    /** By assumption: associative, commutative, idempotent. */
    override def merge(left: Option[A], right: Option[A]): Option[A] = OptionAsLattice[A].merge(left, right)
  }
  ...
  implicit def MapAsUIJDLattice[K, V: UIJDLattice]: UIJDLattice[Map[K, V]] = new UIJDLattice[Map[K, V]] {
    ...
    /** By assumption: associative, commutative, idempotent. */
    override def merge(left: Map[K, V], right: Map[K, V]): Map[K, V] = MapAsLattice[K, V].merge(left, right)
  }
}
